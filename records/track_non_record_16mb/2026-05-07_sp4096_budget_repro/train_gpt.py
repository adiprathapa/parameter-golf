...
class Hyperparameters():
    ...
    artifact_byte_cap = int(os.environ.get('ARTIFACT_BYTE_CAP', 16_000_000))
    artifact_fit_enabled = bool(int(os.environ.get('ARTIFACT_FIT_ENABLED', '1')))
    artifact_fit_tensors = tuple(
        x for x in os.environ.get('ARTIFACT_FIT_TENSORS', 've_shared.embed.weight').split(',') if x
    )
...
def fit_quant_artifact_under_cap(
    ...
    """Coarsen selected quantized tensors until compressed artifact bytes fit."""
    applied: list[str] = []
    max_rounds = 8
    ...
    def pack() -> bytes:
        ...
        return _compress(buf.getvalue(), compressor)

    quant_blob = pack()
    if len(quant_blob) + code_bytes <= cap:
        return quant_blob, applied
    ...
    for _ in range(max_rounds):
        ...
        for name in tensor_names:
            ...
            quant_result[qname] = torch.clamp(torch.round(q.float() / 2), -127, 127).to(torch.int8)
            quant_result[sname] = quant_result[sname] * 2
            applied.append(name)
            changed = True
            quant_blob = pack()
            if len(quant_blob) + code_bytes <= cap:
...
def serialize(h: Hyperparameters, base_model: torch.nn.Module, code: str) -> int:
    ...
    if h.artifact_fit_enabled:
        quant_blob, fit_applied = fit_quant_artifact_under_cap(
            quant_result,
            quant_meta,
            code_bytes,
            h.compressor,
            h.artifact_byte_cap,
            h.artifact_fit_tensors,
        )
    else:
        fit_applied = []
    ...
    if h.is_main_process:
        with open(h.quantized_model_path, "wb") as f:
            f.write(quant_blob)
        if fit_applied:
            log(f"Artifact fit: coarsened {','.join(fit_applied)}")
        ...
        if bytes_total > h.artifact_byte_cap:
            log(f"WARNING: artifact exceeds cap:{bytes_total}>{h.artifact_byte_cap}")
